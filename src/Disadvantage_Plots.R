...
df_success <- df %>% 
  mutate(
    p_hat = unlist(map2(.x = saves, .y = status, 
                  ~ survival_prob(saves = .x, B, rule = "disadvantage", status = .y, reason = "success", roll_time)$prob)), 
    sd = unlist(map2(.x = saves, .y = status, 
              ~ survival_prob(saves = .x, B, rule = "disadvantage", status = .y, reason = "success", roll_time)$sd)), 
...
ggplot(df_success, aes(x=saves, y=p_hat, color = status))+
  ...
  ggtitle("Probability Based on Number of Saves Required", 
          subtitle =  "Scenario: Give Disadvantage Based on Successes")+
...
ggplot(df_fail, aes(x=saves, y=p_hat, color = status))+
  ...
  scale_x_continuous(breaks=seq(0, 16, 2))+
  ggtitle("Probability Based on Number of Saves Required", 
          subtitle =  "Scenario: Give Disadvantage Based on Fails")+
  labs(y = "Probability of survival", x = "Number of Saves Required")+
